...
export default function CasosDeUso() {
  return (
    <section
      id="casos-de-uso"
      className="w-full bg-white py-12 md:py-28 text-zinc-900 border-b border-zinc-100"
    >
      <div className="w-full max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          {useCases.map((item) => (
            ...
            >
              <div className="bg-[#f8f9fa] group-hover:bg-white rounded-[19px] p-5 md:p-7 flex flex-col justify-between min-h-[190px] md:min-h-[260px] h-full w-full transition-all duration-300">
                <div>
                  {/* Badge */}
                  ...
                  {/* Title & Desc */}
                  <h3 className="text-base md:text-[18px] font-bold text-zinc-900 tracking-tight leading-snug">
                    {item.title}
                  </h3>
                  <p className="text-xs md:text-sm text-zinc-500 leading-relaxed mt-2 md:mt-3">
                    {item.description}
                  </p>
                </div>
