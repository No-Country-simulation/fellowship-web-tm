...
- **Descripción:** Pestaña "Comparar equipos", réplica del mockup `showcase-v16-comparar-20-equipos.html`. Arriba, un selector de proyecto (dropdown propio con cruz para volver al inicio); sin proyecto muestra un estado vacío punteado. Con proyecto: a la izquierda el gráfico SVG "Trayectoria semanal" (grilla 0–100, S1–S8, una línea con puntos por equipo elegido, datos de `teamActivity`) y a la derecha los tiles de equipos agrupados por edición. Cada tile muestra el código (`E01`, `E02`…), el score grande con su barra, y abajo (separados por una línea) mensajes, reuniones y entregables con ícono (el nombre de cada dato aparece al pasar el mouse). Se eligen hasta 5 equipos: cada uno toma el primer color libre de la paleta (`#FF0094`, `#02BEEF`, `#A855F7`, `#646CF6`, `#0CFCA7`), el tile se tiñe, la barra toma el color del equipo y aparece un check en la esquina; al llegar a 5 el resto queda deshabilitado. En el gráfico, cada línea nueva se dibuja de izquierda a derecha y sus puntos aparecen uno por uno (clases `.cmp-line` / `.cmp-dot` en `globals.css`, desactivadas con `prefers-reduced-motion`); el último punto va hueco para marcar la semana actual, y al pasar el mouse por un equipo elegido (en su tile o en la leyenda) su línea se resalta y las demás se atenúan.
  - **Leyenda:** debajo del gráfico, un chip por equipo elegido con su color y `E01 · Jul` (el mes va porque `E01` se repite entre ediciones).
  - **Dato de cada semana:** al pasar el mouse sobre el gráfico se marca la semana con una guía punteada, se agrandan sus puntos y aparece un cuadrito con el valor de cada equipo en esa semana, ordenado de mayor a menor. Lo detectan 8 rectángulos invisibles (uno por semana) encima del SVG; en los bordes el cuadrito se corre para no salirse.
  - **Compartir comparación:** botón en el encabezado de "Comparación en detalle" que copia al portapapeles un link del tipo `/sobre-nosotros/showcase?comparar=saludconecta&equipos=sc-jul-1,sc-jun-2`. Los equipos van ordenados por color, así quien lo abre ve cada equipo con el mismo color. El botón muestra "¡Link copiado!" (o "No se pudo copiar") por 2 segundos. Quien abre el link cae directo en esta pestaña con ese proyecto y esos equipos (lo lee `Showcase` y lo pasa en `initial`). Los ids inválidos, repetidos o de otro proyecto se ignoran, con un máximo de 5.

  Debajo, "Comparación en detalle": una tarjeta por equipo elegido (puntaje, integrantes, reuniones, mensajes, entregables, stack y tres accesos: repositorio, página del equipo y Showcase Play). La sección se sale del ancho de 1120px hasta `min(1464px, 100vw - 48px)` para que entren las 5 tarjetas.
- **Props:** `onPlay?: () => void` — lo llama el botón "Ver en Showcase Play" de cada tarjeta (hoy solo cambia a esa pestaña). `initial?: CompareSelection | null` — proyecto y equipos con los que arranca (viene del link compartido); se usa solo al montar.
- **Dependencias:** `next/link`, `lucide-react` (`Check`, `ChevronDown`, `CircleCheck`, `Link2`, `MessageSquare`, `TrendingUp`, `Users`, `Video`, `X`), `@/components/ui/reveal`, `GithubIcon`, `TechChip`, `@/lib/utils`, `@/lib/showcase` (`COMPARE_MAX_TEAMS`, `buildCompareSearch`, `countTeams`, `pluralize`, `showcaseProjects`, `teamActivity`, `teamHref`, `teamMessages`, `teamScore`, tipo `CompareSelection`), `"use client"`.
- **Uso:**
  ```tsx
  import CompareList from "@/components/organisms/sobre-nosotros/showcase/CompareList";
  <CompareList onPlay={goToPlay} initial={compareInit} />;
  ```
